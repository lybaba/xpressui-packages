# Appearance & Design Tokens — Test Presets

This document gathers combinations of variables (Design Tokens) to test in the `Customize Workflow > Appearance` interface of XPressUI Pro.
These presets demonstrate the absolute flexibility of the form without writing a single line of CSS.

---

## 1. Midnight SaaS (Dark Mode)
*Ideal for tech agencies, crypto startups, or developer tools.*

- **Background Style:** Panel Focus
- **Background Image URL:** *(Leave empty or use a dark abstract image:)*
- **Typography (Font):** `"Inter", system-ui, sans-serif`
- **Primary color:** `#6366f1` *(Vibrant Indigo)*
- **Surface color:** `#1e293b` *(Slate 800 - card background)*
- **Page background:** `#0f172a` *(Slate 900 - page background)*
- **Text color:** `#f8fafc` *(Slate 50 - very light text)*
- **Border color:** `#334155` *(Slate 700 - subtle borders)*
- **Card radius:** `20`
- **Input radius:** `10`
- **Button radius:** `10`

---

## 2. Luxury Law Firm (Elegant & Classic)
*Ideal for lawyers, financial consultants, or premium services. A very crisp look, with sharp edges and a Serif font.*

- **Background Style:** None (Clean)
- **Background Image URL:** *(Leave empty or put your own image)*
- **Typography (Font):** `"Playfair Display", "Merriweather", serif`
- **Primary color:** `#b45309` *(Dark Amber / Gold)*
- **Surface color:** `#ffffff` *(Pure White)*
- **Page background:** `#fafaf9` *(Warm Gray / Eggshell)*
- **Text color:** `#171717` *(Deep Black)*
- **Border color:** `#e5e5e5` *(Elegant Light Gray)*
- **Card radius:** `0` *(Perfectly square corners)*
- **Input radius:** `0`
- **Button radius:** `0`

---

## 3. Playful Startup (Neo-Brutalism / Fun)
*Ideal for creative agencies, marketing, or event planning.*

- **Background Style:** Full Bleed
- **Background Image URL:** *(A very colorful or geometric pattern image)*
- **Typography (Font):** `"Space Grotesk", "Comic Sans MS", sans-serif`
- **Primary color:** `#f43f5e` *(Bright Pink / Red)*
- **Surface color:** `#ffffff` 
- **Page background:** `#fef08a` *(Warm Yellow)*
- **Text color:** `#000000` *(Pure Black)*
- **Border color:** `#000000` *(Strong black borders)*
- **Card radius:** `16`
- **Input radius:** `8`
- **Button radius:** `8`

---

## 4. Soft Eco & Health (Wellness & Nature)
*Ideal for clinics, psychologists, or eco-friendly products. Very rounded and soft shapes.*

- **Background Style:** Panel Focus
- **Background Image URL:** *(Nature image or soft texture)*
- **Typography (Font):** `"Nunito", "Quicksand", sans-serif`
- **Primary color:** `#059669` *(Soft Emerald)*
- **Surface color:** `#ffffff`
- **Page background:** `#ecfdf5` *(Very pale mint)*
- **Text color:** `#064e3b` *(Very dark green to replace black)*
- **Border color:** `#a7f3d0` *(Light green borders)*
- **Card radius:** `32` *(Very round)*
- **Input radius:** `999` *(Absolute pill shape)*
- **Button radius:** `999` *(Absolute pill shape)*

---

## 💡 How to test the background image?
To test the `Panel Focus` and `Full Bleed` styles, go to the **Media** menu in your WordPress, upload a beautiful image (e.g., an abstract Unsplash background), click on the image, and copy the "**File URL**" to paste it into the *Background Image URL* field.